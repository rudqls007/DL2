'''
    1. 딥러닝 모델의 학습 과정
         데이터 준비 ==> 모델 (모형) 정의
                            ||
                           \||/
                        모델 (모형) 컴파일 <------ dhqxlakdlwj(optimizer) 선택, 손실함수(loss) 선택
                            ||
                           \||/
                        모델 (모형) 학습 (훈련)
                            ||
    ...
    2. 텐서플로(TensorFlow)
        - 기계 학습 프레임워크(framework) 중 하나임.
            ...
            - 데이터 흐름 그래프(data flow graph)를 사용하여 데이터의 수치연산을 하는 오픈소스 소프트웨어 프레임 워크
            - 넘파이같은 다양한 라이브러리를 묶어 놓은 패키지라고도 할 수 있음
        ...
        - 텐서플로에서 데이터의 수치 연산을 수행하기 위한 그래프
            ...
            - 엣지(edge)는 노드 사이의 관계를 표현하며, 데이터(tensor) 이동을 수행
...
'''
    길이가 100이고 shape가 (3,)인 1차원 배열을 생성한 후, (100, 3) 크기의 2차원 배열로 변환한다.
...
'''
    * GPU 사용 여부 체크하기
        ...
        - 각텐서(tensor)와 연산이 어떠한 장치에 할당되었는지

'''
tf.debugging.set_log_device_placement(True)         # 각 텐서와 연산이 어떠한 장치에 할당되었는지 출력하기
