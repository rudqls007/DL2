'''
    1. 텐서플로 일반적인 아키텍처
        ...
        3) 사용자에게 웹이나 모바일로 서비스를 베포 환경도 제공
    ...
    3. 데이터 준비
        1) 데이턱 ㅏ텍스트일 경우 (텍스트 모델을 사용해야 할 경우)
            - 서로 다른 길이의 시퀀스(sequence)를 분할하여 처리
'''
import os
import numpy as np                  # 벡터 및 행렬 연산에서 맹 ㅜ편리한 기능 제공하는 파이썬 라이브러리 패키지
...
'''
    * 데이터 셋을 열어 보면 특성(컬럼) 일곱 개로 구성됨
        ...
        5) capacity : 수하물 용량
        6) safety : 안정성
        7) output : 차 상태 (unacc - 허용 불가수준, acc - 허용 가능수준, good - 양호, vgood -  매우 좋다)
...
'''
    * 데이터에 대한 전처리
        1) 딥러닝은 통계 알고리즘 기반으로 하기 떄문에 범주 정보를 숫자로 변환해야 함
        2) 원 - 핫 인코딩 (one-hot encoding) 방법 사용
            - 새로운 열을 생성
            -구유값을 갖는 경우 1 값을 부여함
        3) get_dummies()
            - 가변수(dummy variable)를 만둘어 주는 함수
            - 문자를 숫자(0,1)로 바꾸어주는 것.
        4) concat() : 여섯개 열 병함 (문자열로 결합)
'''
...
x = pd.concat([price, maint, doors, persons, capacity, safety, labels], axis=1)
y = labels.values
...
# 케라스 라이브러리 안에 Model 모듈의 함수들 사용한다.
...
# categorical_crossentropy의 출력 값은 원-핫 인코딩된 결과가 출력 됨
model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['acc'])

'''
    모델은 완전 연결층은 총 3개로 구성되어 있다.
'''
...
# 모델 평가
# 테스트 정확도(test_accuracy)의 결과값이 0.96으로, 정확도는 약 97%
# 정확도가 높기 때문에 학습이 잘되었다고 할 수 있다.
